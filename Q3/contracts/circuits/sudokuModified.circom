...
template sudoku() {
    ...
    component puzzleRangeCheck[9][9];
    component solutionRangeCheck[9][9];

    for (var i=0; i<9; i++) {
        for (var j=0; j<9; j++) {
            puzzleRangeCheck[i][j] = RangeProof(5);
            puzzleRangeCheck[i][j].in <== puzzle[i][j];
            puzzleRangeCheck[i][j].range[0] <== 0;
            puzzleRangeCheck[i][j].range[1] <== 9;
            puzzleRangeCheck[i][j].out === 1;

            solutionRangeCheck[i][j] = RangeProof(5);
            solutionRangeCheck[i][j].in <== solution[i][j];
            solutionRangeCheck[i][j].range[0] <== 0;
            solutionRangeCheck[i][j].range[1] <== 9;
            solutionRangeCheck[i][j].out === 1;
